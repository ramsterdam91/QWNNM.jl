...
function denoising(Img_O::Array{Float32,3}, Img_N::Array{Float32,3}, Par::PAR)::Array{Float32,3}
    ...
    Sigma_arr           =   zeros(Float32, 1, TotalPatNum)
...
function im2patch(Img_E::Array{Float32,3}, Img_N::Array{Float32,3}, Par::PAR)
    ...
    SigmaArr = Par.lamada .* sqrt.(abs.(mean((N_Y-Y).^2, dims=1) .- Par.nSig^2 ))
